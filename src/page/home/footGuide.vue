<template>
    <section id='foot_guide'>
        ...
        <router-link tag="div" :to="{name:'msite',path:'/msite' ,query:{geohash:geohash} }"  class="guide_item">
            <svg class="icon_style">
                <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.name=='msite' ? '#msiteActive' : '#msite'"></use>
            </svg>
            <span>外卖</span>
        </router-link>
        <router-link tag="div" :to="{name:'search',path:'/search' ,params:{geohash:geohash} }" class="guide_item">
        	<svg class="icon_style">
                <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.name == 'search' ? '#findActive' : '#find'"></use>
            </svg>
            <span>搜索</span>
        </router-link>
        <router-link tag="div" :to="{name:'order',path:'/order'}" class="guide_item">
        	<svg class="icon_style">
                <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.name == 'order' ? '#orderActive' : '#order'"></use>
            </svg>
            <span>订单</span>
        </router-link>
        <router-link tag="div" :to="{name:'personal_center',path:'/personal_center'}" class="guide_item">
        	<svg class="icon_style">
                <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.name == 'personal_center' ? '#profileActive' : '#profile'"></use>
            </svg>
            <span>我的</span>
        </router-link>
    </section>
